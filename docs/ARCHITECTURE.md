...
flowchart LR
  ...
  B --> C["ComingSoon API: kommende Inhalte"]
...
| comingsoon_user_id | Personalisierte Vorschau, falls verfügbar |
...
| ComingSoon | Veröffentlichungs- und Verfügbarkeitsdaten |
...
Änderungen sollen im geöffneten Dashboard nahezu sofort erscheinen. Deshalb werden Ereignis- oder Webhook-Schnittstellen den reinen Intervallabfragen vorgezogen; die konkret verfügbaren Schnittstellen von Emby, Seerr und ComingSoon werden vor der Umsetzung geprüft.

Die Basis-URLs und geheimen Token für Emby und Seerr werden serverseitig über eine `.env`-Datei konfiguriert und nie an den Browser übertragen.
