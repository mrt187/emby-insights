...
Set `EMBY_BASE_URL` (including `/emby`), a unique `EMBY_DEVICE_ID` and `EMBY_ADMIN_API_KEY` as well. Emby passwords are never stored; after a successful login only the temporary Emby access token is retained in Redis for the dashboard session.
