...
func TestAuthenticateSendsEmbyRequest(t *testing.T) {
	...
	identity, err := NewClient(testServer.URL+"/emby", "dashboard-device").Authenticate(context.Background(), Credentials{Username: "thomas", Password: "secret"})
	if err != nil {
...
func TestAuthenticateRejectsInvalidCredentials(t *testing.T) {
	...
	defer testServer.Close()
	_, err := NewClient(testServer.URL, "dashboard-device").Authenticate(context.Background(), Credentials{})
